...
@Component({
    ...
    templateUrl: './bodyTypes.html',
    providers: [MessageService, ConfirmationService, RequestsService, CountryService]
})
export class BodyTypes implements OnInit {
    bodyTypeDialog: boolean = false;
    bodyType!: BodyType;
    bodyTypes = signal<BodyType[]>([]);

    selectedBodyTypes!: BodyType[] | null;
    ...
    loadBodyTypes(event?: TableLazyLoadEvent | { first: number; rows: number }) {
        ...
        this.requestsService.api('GET', 'body-types', {
            ...
            next: (res) => {
                this.bodyTypes.set(res.body?.['body-types'] ?? []);
            ...
            error: () => {
                ...
                this.messageService.add({
                    severity: 'error',
                    summary: 'Load failed',
                    detail: 'Could not load body types',
                    life: 4000,
                });
            }
        });
    ...
    onGlobalFilter(event: Event) {
        ...
        this.searchTimer = setTimeout(() => {
            ...
            this.loadBodyTypes({ first: 0, rows: this.dt.rows });
        }, 250);
    }

    refreshCurrentPage() {
        this.loadBodyTypes({ first: this.dt.first ?? 0, rows: this.dt.rows ?? 5 });
    }

    openNew() {
        this.bodyType = {};
        this.submitted = false;
        this.bodyTypeDialog = true;
    }

    editBodyType(type: BodyType) {
        this.bodyType = { ...type };
        this.bodyTypeDialog = true;
    }

    hideDialog() {
        this.bodyTypeDialog = false;
        this.submitted = false;
    }

    deleteBodyType(bodyType: BodyType) {
        this.confirmationService.confirm({
            message: `Are you sure you want to delete ${bodyType.name}?`,
            header: 'Confirm',
                ...
                this.requestsService
                    ...
                    .subscribe({
                        next: () => {
                            ...
                            this.bodyType = {} as any;
                            this.messageService.add({
                                severity: 'success',
                                summary: 'Successful',
                                detail: 'Body Type Deleted',
                                life: 3000,
                            });
                        },
                        error: (err) => {
    ...
    deleteSelectedBodyTypes() {
        this.confirmationService.confirm({
            message: 'Are you sure you want to delete the selected engines?',
            header: 'Confirm',
            icon: 'pi pi-exclamation-triangle',
            accept: () => {
                const selected = this.selectedBodyTypes ?? [];
                ...
                this.requestsService.api('DELETE', 'body-types', { body: { ids } }).subscribe({
                    next: () => {
                        ...
                        // clear selection + current bodyType
                        this.selectedBodyTypes = null;
                        this.bodyType = {} as any;

                        this.messageService.add({
                            severity: 'success',
                            summary: 'Successful',
                            detail: 'Body Types Deleted',
                            life: 3000,
                        });
                    },
                    error: (err) => {
                        this.messageService.add({
                            severity: 'error',
                            summary: 'Delete failed',
                            detail: err?.message ?? 'Could not delete selected body types',
                            life: 4000,
                        });
                    },
                });
    ...
    async saveBodyType() {
        ...
        try {
            ...
            if (this.bodyType.id) {
                ...
                this.bodyType = res?.body?.bodyType ?? this.bodyType;
            }
            ...
            this.messageService.add({
                severity: 'success',
                summary: 'Saved',
                detail: 'Body Type saved successfully',
            });
