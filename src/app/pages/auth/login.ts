...
export class Login {
    ...
    onLogin() {
        ...
        this.auth.login({ Email: this.email, Password: this.password }).subscribe({
            ...
            error: (err) => {
                // err might be:
                // - your RequestsService error shape: { status, message, error }
                // - or the thrown backend body: { success:false, error:"..." }
                const msg =
                    err?.error?.error ||   // backend error wrapped by RequestsService
                    err?.error ||          // thrown body: {error:"..."}
